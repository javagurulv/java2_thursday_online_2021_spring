...
public class CreateOfferServiceTest {
    ...
    public void shouldReturnResponseWithErrorsWhenValidationFails() {
        CreateOfferRequest request = new CreateOfferRequest(null, "Category", "Description", 20.0);
    ...
    public void shouldCreateOfferInDatabase() {
        ...
        CreateOfferRequest request = new CreateOfferRequest("Type", "Category", "Description", 20.0);
        CreateOfferResponse response = service.execute(request);
        assertFalse(response.hasErrors());
        Mockito.verify(database).createOffer(
                argThat(new OfferMatcher("Type", "Category", "Description", 20.0)));
    }
}
